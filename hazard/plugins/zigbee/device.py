...
class ZigBeeDevice:
    ...
    def _on_frame(self, addr16, source_endpoint, dest_endpoint, cluster, profile, data):
        ...
        if addr16 != self._addr16:
            LOG.info(
                "Updating addr16 on {} (rx: 0x{:04x}, config: 0x{:04x})".format(
                    self.addr64hex(), addr16, self._addr16
                )
            )
            self._addr16 = addr16
            self._network._hazard.save()
        if (
            profile == zcl.spec.Profile.ZIGBEE
            and dest_endpoint == zcl.spec.Endpoint.ZDO
        ):
    ...
    def _on_zdo(self, cluster, data):
        ...
        LOG.info(
            'ZDO from "{}" ({}): {} {}\n   {}'.format(
                self._name, self._addr16, seq, cluster_name, kwargs
            )
        )
    ...
    def _on_zcl(self, source_endpoint, dest_endpoint, cluster, profile, data):
        (
            ...
            default_response,
            kwargs,
        ) = zcl.spec.decode_zcl(cluster, data)
        LOG.info(
            'ZCL from "{}": {} {} {} {} {} {} {} {} {}'.format(
                self._name,
                source_endpoint,
        ...
        if default_response:
            asyncio.get_event_loop().create_task(
                self._send_default_response(
                    source_endpoint, cluster_name, command_name, zcl.spec.Status.SUCCESS
                )
            )
        if self._on_zcl_callback:
            asyncio.get_event_loop().create_task(
                self._on_zcl_callback(
                    source_endpoint,
                    dest_endpoint,
                    cluster_name,
                    command_type,
    ...
    async def _send_default_response(
        self, endpoint, cluster_name, command_name, status
    ):
    ...
    async def _on_match_descriptors(self, profile, in_clusters, addr16, out_clusters):
        if profile == zcl.spec.Profile.HOME_AUTOMATION and in_clusters == [0x0019]:
            ...
            return
        LOG.warning(
            "Attempted to match descriptors: profile {} / in {} / out {}".format(
                profile, in_clusters, out_clusters
            )
        )
    ...
    async def _send(
        ...
        # print(hex(seq))
        result = await self._network._module.unicast(
            self._addr64,
            self._addr16,
            source_endpoint,
            dest_endpoint,
