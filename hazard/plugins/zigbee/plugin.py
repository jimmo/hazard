...
class ZigBeePlugin(HazardPlugin):
    ...
    def get_routes(self):
        return [
            ...
            aiohttp.web.post(
                "/api/zigbee/device/{device}/thing", self.handle_device_create_thing
            ),
            aiohttp.web.post(
                "/api/zigbee/device/{device}/zdo/{cluster_name}", self.handle_device_zdo
            ),
            aiohttp.web.post(
                "/api/zigbee/device/{device}/zcl/profile/{profile}/{endpoint}/{cluster_name}/{command_name}",
                self.handle_device_profile_zcl,
            ),
            aiohttp.web.post(
                "/api/zigbee/device/{device}/zcl/cluster/{profile}/{endpoint}/{cluster_name}/{command_name}",
                self.handle_device_cluster_zcl,
            ),
            aiohttp.web.post(
                "/api/zigbee/device/{dest_device}/replace/{src_device}",
                self.handle_device_replace,
            ),
            ...
            aiohttp.web.post(
                "/api/zigbee/group/{group}/thing", self.handle_group_create_thing
            ),
            aiohttp.web.post(
                "/api/zigbee/group/{group}/remove", self.handle_group_remove
            ),
            aiohttp.web.post(
                "/api/zigbee/group/{group}/zcl/cluster/{profile}/{endpoint}/{cluster_name}/{command_name}",
                self.handle_group_cluster_zcl,
            ),
        ]
    ...
    async def handle_status(self, request):
        return aiohttp.web.json_response(
            {
                "coordinator_addr64": "0x{:08x}".format(
                    await self._module.get_coordinator_addr64()
                ),
    ...
    async def handle_device_profile_zcl(self, request):
        ...
        kwargs = await request.json()
        result = await device.zcl_profile(
            profile,
            int(request.match_info["endpoint"], 10),
            request.match_info["cluster_name"],
            request.match_info["command_name"],
            **kwargs,
        )
        return aiohttp.web.json_response(result)
    ...
    async def handle_device_cluster_zcl(self, request):
        ...
        profile = self.get_profile_from_request(request)
        kwargs = await request.json()
        result = await device.zcl_cluster(
            profile,
            int(request.match_info["endpoint"], 10),
            request.match_info["cluster_name"],
            request.match_info["command_name"],
    ...
    def find_zigbee_light(self, device):
        for t in self._network._hazard.find_things(
            hazard.plugins.zigbee.things.ZigBeeLight
        ):
            if t._device == device:
                return t
    ...
    async def handle_device_replace(self, request):
        ...
        for group_addr16 in dest_thing._groups:
            ...
            print(
                ...
                dest_thing._endpoint,
                "groups",
                "add_group",
                group._addr16,
                group._name,
            )
            await src.zcl_cluster(
                zcl.spec.Profile.HOME_AUTOMATION,
                dest_thing._endpoint,
                "groups",
                "add_group",
        ...
        print(
            f"update dest device {dest_thing._device._addr16}/{dest_thing._device.addr64hex()} to {src._addr16}/{src.addr64hex()}"
        )
    ...
    async def handle_group_cluster_zcl(self, request):
        ...
        kwargs = await request.json()
        group = self._network.find_group(group_addr16)
        result = await group.zcl_cluster(
            group_addr16,
            profile,
            int(request.match_info["endpoint"], 10),
            request.match_info["cluster_name"],
