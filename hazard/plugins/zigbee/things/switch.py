...
class ZigBeeSwitch(Switch):
    ...
    async def _on_announce(self):
        ...
        coordinator_addr64 = (
            await self._device._network._module.get_coordinator_addr64()
        )

        endpoints = await self._device.zdo("active_ep", addr16=self._device.addr16())
        for ep in endpoints["active_eps"]:
            simple_desc = await self._device.zdo(
                "simple_desc", addr16=self._device.addr16(), endpoint=ep
            )
            desc = simple_desc["simple_descriptors"][0]
            clusters = desc["out_clusters"]
            LOG.info(
                "Discovered output clusters on endpoint {}: {}".format(ep, clusters)
            )
