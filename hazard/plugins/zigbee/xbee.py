        ...
        if self._xbee_module._escape:
            if self._data[i] == 0x7E:
                ...
                if chk_expected == chk_actual:
                    ...
                    LOG.error(
                        "bad escape frame checksum at %d: %s from %s",
                        i,
                        frame,
                        self._data[:i],
                    )
...
class XBeeModule(ZigBeeModule):
    ...
    def connect(self):
        ...
        coro = serial_asyncio.create_serial_connection(
            loop, self._create_protocol, self._port, baudrate=self._baudrate
        )
    ...
    async def unicast(
        ...
        return await self._tx_explicit(
    ...
    def _on_frame(self, data):
        ...
        if frame_type in (
            ...
            LOG.error(
                "Unknown frame type: 0x{:02x}".format(
                    frame_type,
                )
            )
    ...
    async def _send_frame(self, frame_type, data, timeout=5, status=True):
        ...
        if self._escape:
            ...
            data = (
                ...
                + data
                + struct.pack("B", self._protocol._checksum(data))
            )
        # print('sending', data)
